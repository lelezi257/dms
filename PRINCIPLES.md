...
4. **Meta 只管粗粒度权威。** Meta/node 是控制结构。Meta 持久管理节点、workspace 的 Home 归属、镜像私有 draft 的单写者归属、已发布镜像版本及副本位置，并为管理面与亲和调度提供查询。它不参与 workspace 的逐文件操作，也不参与已发布镜像的逐块读取。首版以 etcd 支撑权威条件事务、选主和持久化，预留内嵌 Rust Raft 的 MetaStore 后端；单节点与三节点多数派模式分别声明容错，不自制另一套双份控制日志。控制面失去提交能力时暂停新归属、授权与发布，不自动选第二个数据位置。
5. **Workspace 本地路径尽量薄。** Home 仅表示根的数据所在节点，不是独立进程。WorkspaceRoot 是归属与授权单位；首次访问从 Meta 获取 RootGrant，根内操作复用它。其他节点读写前，必须先让原节点撤销独享、完成缓存/在途请求屏障并 ACK，再由 Meta 提交新授权。Home 节点在普通本机文件系统保存目录和文件；本地 FUSE 尽量透传，避免每次写都同步做远端发布、分片或新 Block。远端经 P2P 访问同一份 Home 普通文件，并维持共享读写的正确性；共享不触发 Blob 转换。只有显式镜像/快照发布才由镜像后端从稳定副本生成 Blob，原 workspace 仍是普通文件。Home 迁移、共享冲突和故障可走较慢路径，但不能静默丢失或错读已确认的数据。
...
10. **以现状和证据约束宣称。** 新分支当前已有可运行基础框架：Meta/Node 进程、配置、观测、REST/gRPC ping、Local SDK UDS+SHM、Node→Node control/data 8 字节诊断与 FUSE namespace 分派已经接线并通过 Linux 验证；完整 OwnerFs workspace 与 BlobFs 镜像发布尚未实现。需求与架构设计已经明确性能目标、恢复合同与验收矩阵，但旧分支实验和本轮基础诊断不能自动写成文件系统交付结论。
