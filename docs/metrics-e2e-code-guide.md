...
| 所有者 | 主要观察什么 | 定义/更新入口 |
| --- | --- | --- |
| Client | 用户操作次数/耗时/inflight、payload、会话、Region 映射复用/数量 | [Client metrics](../sdk/rust/dms-client/src/metrics.rs) |
...
SDK 不再提供 `dms_client_cache_lookups_total` / `dms_client_cache_invalidations_total`，不是让它们永久报零。`dms_client_region_mapping_lookups_total` 仍表示 FD/mmap 复用，不能解释成 value 命中率。Node Current 布局命中和 Peer 拉取次数分别用于观察权威解析复用和已拉取 Block 的复用。
