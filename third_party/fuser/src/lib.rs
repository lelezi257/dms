...
pub trait Filesystem {
    ...
    /// Cancel an earlier request identified by its FUSE unique id.
    ///
    /// Implementations should reply successfully after arranging cancellation. If the
    /// original request is not known yet or has already completed, reply with `EAGAIN`;
    /// the kernel will either retry the interrupt or ignore the late response.
