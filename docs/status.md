...
**设计已完成、实现未开始：** [需求分析](requirements.html)列出 W0–W5 / I0–I5、首版操作覆盖和验收口径；[详细架构](architecture.html)确定单 FUSE 入口、独立双后端、粗粒度 Master、近计算部署、Home 本地文件/P2P 直连、镜像稳定切点/不可变 Blob/两故障域副本与控制记录。两份 HTML 完成三轮自审；它们是设计决策，不是当前代码能力或 Linux 性能结果。
...
**用户修订：** 取消 NFS 产品后端；近计算与 P2P 升为宪法原则。Workspace 在本地和远端共享时均保持普通文件；只有显式镜像/快照发布才生成 Blob。本轮仅更新设计，未实现新服务。
