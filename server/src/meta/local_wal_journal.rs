...
use super::metadata_journal::{
    JournalEntry, JournalError, JournalRecord, MetaSnapshot, MetadataJournal, SnapshotOperation,
    SnapshotReplica, SnapshotReplicaOperation, SnapshotSession,
};
...
fn encode_record(record: &JournalRecord) -> Result<Vec<u8>, JournalError> {
    ...
    match record {
        ...
        JournalRecord::VersionCommitted {
            ...
            put_u8(&mut out, RECORD_VERSION_COMMITTED);
        ...
        JournalRecord::VersionsCommitted { commits } => {
            put_u8(&mut out, RECORD_VERSIONS_COMMITTED);
    ...
    match input.u8()? {
        ...
        RECORD_VERSIONS_COMMITTED => {
            let commits = (0..input.u32()?)
                ...
                .collect::<Result<Vec<_>, JournalError>>()?;
            Ok(JournalRecord::VersionsCommitted { commits })
        }
