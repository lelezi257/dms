...
Node 另有默认 8 MiB 的 Current 元数据缓存，保存布局与同次解析的位置提示，不复制 value。授权和 Watch 有效时，所需 Block 本地命中便直接读取；缺块可按提示拉取，位置失效则按固定版本刷新。默认 TTL 上限1秒，从解析请求发起时算起，不从下载结束重新计时；失效立即撤销资格，不是允许读旧值1秒。可配置关闭，旧 Meta 未授予资格时不启用。
