...
协商 FUSE ABI 7.31 与 `max_pages` 后，1 MiB 写当前由一个有效 write callback 承载，
因此只产生一个 Block、一个 Commit；跨节点首次接管也只 Pull 一个 Block。新建文件仍有
1 次 negative Lookup、1 次 Create 和 1 次 Commit：前两次分别是内核确认名字不存在与发布
空 inode，最后一次来自随后独立的 write callback。若希望合并 Create/Commit，必须先设计
writeback、脏页失败语义和回收，不得在性能修改中暗改。
...
> 六轮对称交替顺序，每轮 220 个文件（其中 30 个 1 MiB 文件），并要求每个关键 case 至少
> 100 个样本。门禁使用六轮同场配对的中位数：读路径按比值要求领先，write-through
> mutation 按绝对微秒增量限制固定控制成本；不能用单轮抖动或小文件百分比失真决定发布。
